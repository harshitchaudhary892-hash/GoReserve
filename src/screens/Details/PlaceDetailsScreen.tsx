...
import { View, Text, StyleSheet, ScrollView, Image, TouchableOpacity, StatusBar, Linking, Platform, Alert, Share } from 'react-native';
...
export const PlaceDetailsScreen: React.FC<PlaceDetailsScreenProps> = ({ navigation, route }) => {
  ...
  useEffect(() => { if (place) return; const loadPlace = async () => { try { const p = await getPlaceById(route.params.placeId); setPlace(p); } catch (error) { Alert.alert('Error', 'Failed to load place details.'); navigation.goBack(); } finally { setLoading(false); } }; loadPlace(); }, [route.params.placeId]);

  const handleFavoriteToggle = useCallback(async () => { if (!user || !place) return; try { if (isFavorite) await removeFromFavorites(user.uid, place.id); else await addToFavorites(user.uid, place.id); await refreshUserProfile(); } catch (error) { Alert.alert('Error', 'Failed to update favorites.'); } }, [user, isFavorite, place]);
  const handleCall = useCallback(() => { if (!place?.phone) { Alert.alert('Info', 'No phone number available.'); return; } const phoneUrl = `tel:${place.phone.replace(/\D/g, '')}`; Linking.canOpenURL(phoneUrl).then((s) => { if (s) Linking.openURL(phoneUrl); else Alert.alert('Error', 'Phone calls are not supported on this device.'); }); }, [place?.phone]);
  const handleDirections = useCallback(() => { if (!place) return; const url = `https://www.google.com/maps/dir/?api=1&destination=${place.latitude},${place.longitude}&travelmode=driving`; Linking.openURL(url); }, [place]);
  const handleShare = useCallback(async () => { if (!place) return; try { await Share.share({ message: `Check out ${place.name} on Go Reserve!\n\n${place.description}\n📍 ${place.address}\n⭐ ${place.rating} | ${place.priceRange}` }); } catch (error) {} }, [place]);
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.section}><Text style={styles.sectionTitle}>Location & Contact</Text><TouchableOpacity style={styles.infoRow}><Ionicons name="location-outline" size={18} color={COLORS.primary} /><Text style={styles.infoText}>{place.address}, {place.city}, {place.state}</Text></TouchableOpacity>{place.phone ? (<TouchableOpacity style={styles.infoRow} onPress={handleCall}><Ionicons name="call-outline" size={18} color={COLORS.success} /><Text style={[styles.infoText, styles.linkText]}>{formatPhoneNumber(place.phone)}</Text></TouchableOpacity>) : null}</View>
        <View style={styles.section}><Text style={styles.sectionTitle}>Map</Text><TouchableOpacity style={[styles.mapContainer, mapExpanded && styles.mapExpanded]} onPress={() => setMapExpanded(!mapExpanded)} activeOpacity={0.95}><MapView style={styles.map} initialRegion={{ latitude: place.latitude, longitude: place.longitude, latitudeDelta: 0.01, longitudeDelta: 0.01 }} scrollEnabled={false} zoomEnabled={false} rotateEnabled={false} pitchEnabled={false}><Marker coordinate={{ latitude: place.latitude, longitude: place.longitude }} title={place.name} description={place.address} /></MapView><View style={styles.mapOverlay}><Text style={styles.mapTapText}>Tap to expand map</Text></View></TouchableOpacity></View>
        {place.amenities && place.amenities.length > 0 && (<View style={styles.section}><Text style={styles.sectionTitle}>Amenities</Text><View style={styles.amenitiesGrid}>{place.amenities.map((amenity, index) => (<View key={index} style={styles.amenityItem}><Ionicons name="checkmark-circle" size={18} color={COLORS.success} /><Text style={styles.amenityText}>{amenity}</Text></View>))}</View></View>)}
        <View style={{ height: SPACING.xxxl }} />
      </ScrollView>
      <View style={styles.bottomBar}><View style={styles.bottomInfo}><Text style={styles.bottomPrice}>{place.priceRange}</Text><Text style={styles.bottomStatus}>{place.availability}</Text></View><Button title="Get Directions" onPress={handleDirections} size="large" icon={<Ionicons name="navigate" size={18} color={COLORS.white} />} style={{ flex: 1 }} /></View>
    </View>
  );
};
